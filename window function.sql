...
select customer_id, customer_name,order_id, order_date, row_number() over(partition by customer_id) from orders ;
...
round(amount-avg(amount) over(partition by category) )
 ...
 select * from 
( select customer_name,  order_id, amount, max(amount) over(partition by customer_id ) as customer_max
 from orders)as t
 ...
 -- or
  ...
  select * from 
( select customer_name,  order_id, amount, rank() over(partition by customer_id order by amount desc) as customer_rank
 from orders)as t
...
( select customer_name,  order_id, amount, rank() over(partition by customer_id order by amount desc) as customer_rank
 ...
 select customer_name, order_id, amount ,  sum(amount) over(partition by customer_id),
round( amount / sum(amount) over(partition by customer_id) *100,2)
 ...
 -- lag / lead 
 select * from orders;
 -- lead age wali values
  select *, lead(amount , 1) over() from orders;
  select *, lead(amount , 1) over() , lead(amount , 2) over() from orders;
   select *, lead(amount , 2) over() from orders;
   
   -- lag piche wali values
   select *, lag(amount ,1) over() from orders; 
   select *, lag(amount , 1) over() , lag(amount , 2) over() from orders;
   
      select *, lag(amount , 1,100) over() , lag(amount , 2,100) over() from orders; 
      
      select *  , lead(amount,1)over(partition by customer_id order by order_id) from orders;
      
      select *, lag(amount,1,0) over(partition by customer_id order by order_id ) as prev2, 
      lag(amount,2,0) over(partition by customer_id order by order_id ) as prev3,
      amount + lag(amount,1,0) over(partition by customer_id order by order_id ) +
      lag(amount,2,0) over(partition by customer_id order by order_id ) from orders;
      ...
      select customer_id , customer_name,amount,
      sum(amount) over(rows between 3 preceding and current row )
      from orders;
      ...
      sum(amount) over( partition by customer_id order by order_date , amount 
